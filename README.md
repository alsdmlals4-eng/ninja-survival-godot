...
최신 작업 상태는 [Active Context](docs/ACTIVE_CONTEXT.md), 승인 규칙은
[현재 결정](docs/CURRENT_CONFIRMED_DECISIONS.md)과
[구현 명세 R01–R10](docs/design/NINJA_SURVIVAL_IMPLEMENTATION_PACKET.md)을 읽는다.
특정 과거 커밋을 현재 main으로 간주하지 않는다. PR #147 개발 후보와 병합된 main은
별개이며 아래 과거 이력은 현재 후보 전체의 완료 증거가 아니다.

### 현재 개발 후보에서 직접 확인할 순서

1. `새 게임` → 시작 유파 → 인법을3개 중1개씩 두 번 →3×3배치 확인 → 준비 확정.
2. 시작 유파와 별개로 첫 전장을 고른다. 이동/WASD·방향키, 무적대시/Space,
   오의/E는 직접 조작하고 일본도·수리검·배치된 인법은 자동 공격한다.
3. 엘리트 처치 → 흔적 회수 → 보스 경고/등장 → 보스 처치로 휴식에 진입한다.
4. 휴식에서 흔적 흡수(해당 유파 인법 획득 자격) 또는 장비에 유파 힘 부여를 고른다.
   시작 유파의 흔적은 강화만 가능하고 이미 고른 시작 인법은 유지된다.
5. 보스 보상·상자를 처리하고 구매/가방 배치/장비 변경/조합을 확인한다.
   엽전 수치 강화는 유파 힘 부여와 별개이며 실패해도 기존 등급·장비는 보존된다.
6. 다음 미방문 전장과 Fate를 고르고 `출전 확정`한다. 네 전장 뒤 최종 보스에 진입한다.
7. 각성에서 닌자소울3개로 다음 새 게임의 지원품 선택을 해금한다. 지원품은 가방을
   차지하고 판매0이다. 저장에 문제가 있으면 `저장 복구 기록 확인`에서 원본 보관 후
   사용할 후보를 직접 고른다. 임시 기록을 확정 저장으로 자동 선택하지 않는다.

8. 제목/전투 멈춤의 설정에서 음량·플레이어 효과 농도·피격 흔들림·조작 안내·화면 모드를
   조절하고 적용한다. 메뉴를 닫을 때 누르고 있던 대시/오의는 손을 뗀 뒤 재개한다.

96경로 자동 시험은 실제 Main/저장/최종정산까지 통과했지만 시간·피해를 가속한다.
실제 일반 속도 완주,96경로 포인터/기기 입력,
사람 재미·최종 아트·기기·출시 검증은 별도 잔여이며 전체 완성을 의미하지 않는다.
현재 selected profile은 `user://ninja_profile_v2.json`이다. 기존 v1 파일은 보존한다.

### 과거 T16 병합 이력 (현재 frontier가 아님)
