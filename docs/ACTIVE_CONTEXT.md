...
base_main_observed_current_planning: 315c66eea9614c284b9c11c4d522141065dfa4b0
...
The user's approval of the hybrid content recommendation and continuous planning work is **not** the separate `기획 완료` declaration.
...
Project GDD Sheets are readable in the current connection, but the 2026-08-11 write retry again returned `403 PERMISSION_DENIED`. No attempted cell changes were applied.

The stale/needs-sync areas observed by readback include:

- `백팩`: old staged wording that puts item rotation/shape later than the approved MVP-4 start scope.
- `인법과조합`: `독침술+은신술`, `보법+뇌둔` are still shown next to the 3 current combos without an MVP-4-vs-future label.
- `변경메모`: historical `5/10/15분` flow remains as history and needs a newer superseding log entry, not destructive rewriting.
- `벤치마킹`: needs the current Backpack Battles / Backpack Hero / DRG Survivor / Sproggiwood / Resogun evidence rows and DEC-002 conclusion.
...
- exact balance values remain tunable `RECOMMENDED_DEFAULT`, not immutable core rules.
...
- DEC-002 item/spatial/reward defaults are planning data only and do not exist in production catalog/runtime.
...
No MVP-4 runtime PASS claim is valid yet. The formative human targets in the content-balance document are still `NOT_RUN`.
