...
pr135_current_main_reconciliation: LOCAL_MACHINE_AND_SCOPED_RUNTIME_CANDIDATE_VERIFIED_PR_CI_PENDING
...
pr135_local_machine_evidence: GODOT_4_7_1_IMPORT_MAIN_SMOKE_FOCUSED_GUT_55_OF_55_585_ASSERTS_FULL_GUT_605_OF_605_6789_ASSERTS_PASS
...
dec039_horde_basic_weapons_and_starting_ninjutsu: USER_APPROVED_ASSET_LOCKED_CURRENT_MAIN_RECONCILIATION_CANDIDATE_LOCAL_MACHINE_VERIFIED_PR_CI_PENDING
...
dec040_four_school_encounters_and_scroll_ninjutsu: USER_APPROVED_CURRENT_MAIN_RECONCILIATION_CANDIDATE_WITH_THREE_USER_LOCKED_BONGMA_RUNTIME_VISUALS_LOCAL_MACHINE_VERIFIED_PR_CI_PENDING
...
dec041_title_front_door_and_four_traditions_medal: USER_APPROVED_SEPARATE_MEDAL_AND_MEDAL_FREE_BACKDROP_LOCKED_CANON_REGISTERED_CURRENT_MAIN_RECONCILIATION_CANDIDATE_LOCAL_MACHINE_VERIFIED_PR_CI_PENDING
...
dec042_title_actions_awakening_and_codex: USER_APPROVED_CURRENT_MAIN_RECONCILIATION_CANDIDATE_LOCAL_MACHINE_VERIFIED_PR_CI_PENDING
...
dec037_runtime_design_review_status: USER_APPROVED_DASH_INVULNERABILITY_CURRENT_MAIN_RECONCILIATION_CANDIDATE_MACHINE_AND_SCOPED_RUNTIME_VERIFIED_PR_CI_PENDING
...
The user-approved source head `d65a712d441d3ca854ee8ae2edff468bb4974983` remains read-only. Its selected runtime/title/encounter deltas are being carried by a fresh branch from completed `origin/main` `7b94efa90ec8c577edf0317163c4cf6b30a32531`, rather than by rewriting the original PR. On this current-main candidate, Godot 4.7.1 import, main-scene smoke, focused GUT `55/55` (`585` assertions), and full GUT `605/605` (`6,789` assertions) pass. A fresh editor also rendered the title and Stage selector, reloaded the defined input map, and exposed the initial horde tree without source errors.

This reconciles only the current candidate's local evidence. Earlier `UNMERGED`, old exact-head, and old count statements below are retained as source lineage, not as proof of the final integration. GitHub CI, merge, post-merge main readback, Human Usability, Player Experience, touch/gamepad/device/export, and balance remain `NOT_RUN` or pending as explicitly stated.
