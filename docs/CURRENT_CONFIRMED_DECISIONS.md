...
## 2026-09-21 추가 확정 — 경험치 성장과 실제 전투 이미지 연결

사용자가 처치 경험치와 레벨업 인술 획득·강화를 명시 요청했다. 아래의 이전
‘경험치 확인 중’ 기록과 R-REWARD의 비도입 조항은 이번 요청으로 대체한다.
일반/엘리트/보스 처치 경험치1/8/20, 레벨 비용12부터6씩 증가, 인술 최대5단계는
초기 시험 수치다. 구슬의 기존 기록 역할과 별개로 처치 시 직접 누적한다.
레벨 선택은 일시정지하고 해금·가방 공간·기존 활성 인법 제한을 준수한다.
장비3종의 별도 슬롯, 흔적 흡수/강화, 모닥불 거래·출전 원자성은 유지한다.
대기 자세·기존 인법/장비 이미지·유파 몬스터·검기/수리검의 실제 사용 연결을
요청했다. 기존 후보를 우선 재사용하고 누락된 대기 자세만 신규 제작한다.
이는 런타임 시험 연결 승인이지 개별 이미지 최종 LOCK이나 Human PASS가 아니다.
전체화면 대시/바닥, 좌상단 실제 인술 재사용 시간과 레벨 표시를 함께 교정한다.

## 2026-09-21 — 시작 전장 자동화와 전투 정보 요청

신규 게임의 스테이지1은 시작 유파와 동일하게 자동 결정한다. 이전 ‘독립 첫 전장
선택’ UX를 대체하지만 이미 저장된 런의 경로와 v1/v2 저장 호환성은 유지한다.
두 인법 선택에 효과 안내, 대시/오의 마우스 도움말, 좌상단 수치 체력/오의,
각 적 하단 붉은 HP와 플레이어 대기 모션, 인법/장비 이미지 제작을 요청했다.
경험치는 현행 R-REWARD의 ‘별도 경험치 없음’과 충돌하므로 확인 중이다.
구슬을 XP로 재해석하거나 레벨업 경제를 임의 추가하지 않는다. 내용 없는8번은 미정.

## Latest 2026-09-20 — 직접 플레이 가능한 네 전장·최종전 구현 승인

사용자는 직전 권장안(유파 고유 효과와 엽전 수치 강화 분리, 실패 시 엽전만
소모하고 장비/단계/부여 효과 보존)을 승인했다. 새 게임부터 네 유파 전장,
엘리트/보스, 모닥불 정비와 구매/강화, 최종 보스까지 직접 플레이할 실행본을
목표로 이어간다. 필요한 상세 데이터·이미지는 조사 후 권장안으로 구체화하도록
위임했다. 같은 범위의 수치·기술 계획을 별도 승인 문서로 반복 차단하지 않는다.
새 과금·플러그인·파괴적 저장 변환·미확인 자산 최종 승인·Human PASS는 제외한다.
구체 규칙과 시험값은 상세 규칙 R-CAMP, 실행 순서는 Implementation Packet R02에
누적한다. 실행 가능성과 사람의 실제 완주·재미 검증은 분리한다.

## 2026-09-20 이전 추가 요청 — 바로 위 구현 승인으로 이어진 이력

사용자 추가 지시: 유파 강화는 선택 장비에 그 유파의 힘을 더하는 것이며,
유파 흡수는 해당 인술의 획득 자격을 연다. 예시는 귀인류→일본도 흡혈,
흑영류→회피 확률, 천술류→수리검 확률 속성 부여다. 이와 별도로 모닥불에서
런 재화(엽전)를 소모하는 확률형 장비 수치 강화, 인술 구매, 가방 강화,
소모품 구매가 가능해야 한다. 영구 닌자소울과 엽전을 합치지 않는다.

이는 기존 R-TRACE의 단순 장비 단계+1과 ‘강화는 흔적만 공급’ 규칙을 수정하는
제품 방향이다. 새 효과/확률/가격/실패/중첩/소모품 사용/저장 호환 세부안은
당시 승인·구현 완료가 아니었다. 이후 승인은 위 최신 항목을 따르고 실제 구현
상태는 Active Context에서 확인한다. 기존 구현을 새 요구 충족으로 표시하지 않는다.
후속 연결: 상세 규칙 R-TRACE/R-EQUIPMENT 및 Implementation Packet의
‘2026-09-20 추가 요청 대조’. 기존 저장·전투 수치에 임의 마이그레이션하지 않는다.

## Latest 2026-09-20 — approved PR147/R01 continuation

User approved the proposed first bundle: reconcile this game-development branch
with latest main's operating contract, then finish save recovery publication and
original preservation. Existing gameplay approvals remain; no retuning, art change,
real save mutation or automatic Draft merge. R02 transactions/Main UI remain sequenced
follow-ups. The current two-review contract supersedes historical five-loop wording.
...
pr135_scoped_runtime_evidence: FRESH_EDITOR_TITLE_AND_STAGE_RENDER_INPUT_MAP_AND_INITIAL_HORDE_TREE_OBSERVED_NO_SOURCE_ERRORS
pr135_human_player_device_export_balance: NOT_RUN
dec039_horde_basic_weapons_and_starting_ninjutsu: USER_APPROVED_ASSET_LOCKED_MERGED_MAIN_PR139_POST_MERGE_MACHINE_VERIFIED
...
dec040_four_school_encounters_and_scroll_ninjutsu: USER_APPROVED_MERGED_MAIN_PR139_WITH_THREE_USER_LOCKED_BONGMA_RUNTIME_VISUALS_POST_MERGE_MACHINE_VERIFIED
...
dec041_title_front_door_and_four_traditions_medal: USER_APPROVED_SEPARATE_MEDAL_AND_MEDAL_FREE_BACKDROP_LOCKED_CANON_REGISTERED_MERGED_MAIN_PR139_POST_MERGE_MACHINE_VERIFIED
...
dec043_title_medal_secondary_scale: USER_APPROVED_2026_09_02_KST_SEPARATE_MEDAL_KEPT_AT_APPROX_NIN_GLYPH_HEIGHT_MERGED_MAIN_PR141_D0E49D0685803849E9013F482F7452830ABBF5D4
dec043_machine_evidence: RED_FOCUSED_GUT_0_OF_1_45_OF_48_ASSERTS_THEN_GREEN_FOCUSED_GUT_1_OF_1_48_ASSERTS_GODOT_4_7_1_IMPORT_EDITOR_PARSE_MAIN_SMOKE_AND_FULL_GUT_605_OF_605_6794_ASSERTS_PASS
dec043_runtime_render_human_player_device: NOT_RUN_NO_EXACT_NINJA_SURVIVAL_HERA_EDITOR_SESSION
...
dec042_title_actions_awakening_and_codex: USER_APPROVED_MERGED_MAIN_PR139_POST_MERGE_MACHINE_VERIFIED
...
dec037_runtime_design_review_status: USER_APPROVED_DASH_INVULNERABILITY_MERGED_MAIN_PR139_MACHINE_AND_SCOPED_RUNTIME_VERIFIED
...
## 2026-09-02 PR #135 current-main reconciliation evidence boundary

The user-approved source head `d65a712d441d3ca854ee8ae2edff468bb4974983` remains read-only. Its selected runtime/title/encounter deltas were brought from completed `origin/main` `7b94efa90ec8c577edf0317163c4cf6b30a32531` through PR #139 and squash-merged as `3428f916f20f545284c337c7eb41b0eacf268351`, without rewriting the original PR. The candidate's focused GUT `55/55` (`585` assertions) passed before PR creation; exact-head GitHub GUT and Windows internal-build checks then succeeded. On the post-merge `main`, Godot 4.7.1 import, main-scene smoke, and full GUT `605/605` (`6,789` assertions) pass. A fresh editor also rendered the title and Stage selector, reloaded the defined input map, and exposed the initial horde tree without source errors.

Earlier `UNMERGED`, old exact-head, and old count statements below are retained as source lineage, not as proof of the current merge. The PR #139 CI, merge, and post-merge main readback are complete. Human Usability, Player Experience, touch/gamepad/device/export, and balance remain `NOT_RUN` as explicitly stated.
