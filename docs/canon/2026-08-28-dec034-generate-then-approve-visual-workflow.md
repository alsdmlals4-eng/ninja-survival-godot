...
- 생성 후보는 `GENERATED_EXPLORATION` 또는 `CANDIDATE`다. user `LOCK` 전에는 approved project asset, Godot 적용, runtime/UX evidence, repository asset manifest 완료로 승격하지 않는다.
...
- `LOCK`된 runtime asset은 repository source + SHA-256/provenance manifest, actual consumer, import/runtime-validation gate를 따른다.
...
`NO_BASE_PROMOTION`: 생성 사전승인 생략의 허용 범위, one-result cadence, project asset lock, and repository/runtime handoff boundary가 현재 프로젝트의 visual workflow 및 user preference에 결속된다.
