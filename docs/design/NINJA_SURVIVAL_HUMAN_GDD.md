...
![잠금된 자동 전투 핵심 장면 참고](../visual/screen-references/scrref-battle-autocombat-continuous-floor-v3.png)
...
## 연속 바닥과 독립 소품
- 전장의 바탕은 화면 밖으로도 계속 이어지는 **연속 바닥**이다. 길을 막는 벽이나 배경 전용 건축물이 플레이 공간의 끝처럼 보이지 않는다.
- 등잔, 고목, 돌, 관목은 배경에 그려 넣지 않고 바닥 위에 드물게 놓이는 독립 소품이다. 그림자와 점유 공간이 따로 읽혀야 한다.
- 이 장면은 잠금된 사람용 전장 기준이며, 아직 실제 게임 화면이나 런타임 교체본은 아니다.
...
## 영구 재화: 닌자소울
- **닌자소울**은 Run 밖에 남는 영구 재화다. 각 스테이지 보스를 넘었다고 즉시 받는 보상이 아니라, Final Binding과 최종 재앙 Boss까지 마친 **진짜 Run 종료**에서 한 번 정산된다.
- 유효한 Workbench 체크포인트와 닌자소울 1이 있을 때만, 그 Run 안에서 **제한된 한 번의 같은 스테이지 재도전**을 선택할 수 있다. 재도전은 실패한 스테이지의 임시 보상·Trace·진행을 되돌리며, Boss 보상이나 닌자소울을 중복 지급하지 않는다.
- 현재 기계 검증 범위에는 재도전 지갑과 체크포인트만 포함된다. 진짜 Run 종료 정산과 결과 화면의 플레이어 경험은 최종 패키지에서 별도로 검증한다.
...
![역사적 달빛 전장 배경 참고](../../assets/runtime/visual-core/moonlit_battlefield_backdrop_v1.png)
...
## 현재 브랜치와 사람용 기준의 경계
- 이 페이지의 달빛 배경은 이전 전장 배경의 **역사적 참고/provenance**다. 현재 브랜치의 게임 화면이 이 이미지 자체를 직접 소비하지는 않는다.
- 앞의 전투 HUD 페이지의 연속 바닥 시안은 사용자가 잠근 사람용 기준이다. 이 기준에서 분리한 연속 바닥 타일, 독립 소품, 접지 그림자는 현재 isolated 브랜치의 실제 전장 소비자에 연결되어 있다.
- 여기까지 확인된 것은 소스·import와 범위가 한정된 런타임 렌더/입력 증거다. 사람 사용성, 플레이 경험, 기기/export, 병합 검증은 아직 하지 않았다.
...
- 자동전투 전장의 배경은 연속 바닥으로 읽히고, 등잔·고목·돌 같은 요소는 독립 소품으로 분리한다.
