...
첫 요약과 Active Context를 따른다. 정상속도 완주는 미완료다. 당시 미완료로 적은
전체5회 검토 요건은 2026-09-20 작업 계약 §7의 동일 계약 2회 예산으로 대체됐다.
...
Main의 기본 신규 시작은 SelectedRunSession을 통해 실제 두 권 선택·시작 유파와 같은
1스테이지 자동 진입·네 준비·최종 재앙으로 이어진다. SchoolCircuit의 selected 모드는 옛 자동
...
| 현재 실제 구현 | 필요한 변경 / 보호할 것 |
|---|---|
| EquipmentLoadoutState.upgrade_equipped는 rank0..4만 증가; 무기+15%/닌자복+3%p | 유파 부여 기록과 유료 수치 rank를 분리. 현재 rank를 어느 축으로 옮길지 호환 설계 전에는 저장 변경 금지 |
| TraditionAccessState/RestCommitCoordinator에 selected 흔적 양자택일·저장 API; Main 미연결 | 획득자격 해금은 재사용; 강화 분기를 고유 효과로 교체하고 기존 한 번 처리·시작 유파 예외 유지 |
| 기존 상점/가방 거래 존재; selected 혼합 품목 R04 미완료 | 엽전/확률/결과/품목 수령을 기존 저장 책임자에서 함께 확정. 새 전역 save/economy manager 금지 |
| 소모품 구매·사용 계약 없음 | 자동 사용 또는 휴식 즉시 사용을 제안하여 하단 수동 스킬바를 만들지 않음. 목록/보관/소비 조건 승인 전 추가하지 않음 |

권장 접근(아직 제품 세부 승인 아님): 유파 효과는 흔적 선택으로 확정 지급,
엽전 강화만 성공 확률 적용; 실패는 장비 파괴/단계 하락 없이 비용만 소모.
UI에 전후 수치·확률·가격·실패 결과를 표시하고 실패 보정 여부는 명세에서 확정한다.
가방 강화는 기존 공간 확장/주머니 구매를 재사용하며 독립 가방 레벨을 만들지 않는다.
대안: 모든 수치 강화 확정 지급은 최신 확률 요청과 달라 REJECT;
파괴/단계 하락형은 짧은 런의 빌드 상실 비용 때문에 REJECT;
비파괴 확률형과 명시적 결과/비용 저장을 ADAPT 권장.

추가 조사: [Brotato 공식 소개](https://store.steampowered.com/app/1942280/Brotato/)의
전투 사이 자원 지출/빌드 선택은 ADAPT, 품목·가격 복사는 REJECT.
[Last Epoch 개발자 회고](https://forum.lastepoch.com/t/crafting-changes-coming-to-eternal-legends-update-0-8-4/45597)의
누적 실패 체감/파괴 위험 문제와 결과 표시 개선은 REFERENCE_ONLY;
그 게임의 성공 보장 제작을 이번 확률 강화 규칙으로 가져오지 않는다.
이는 설계 근거이며 우리 게임의 재미·경제 검증 결과가 아니다.

구현 순서 제안: 효과/실패/중첩/가격 계약 → 분리 장비 스냅샷과 옛 저장 호환 →
흔적 거래·모닥불 구매/확률 거래 → 확정 장비 전투 효과 → 휴식 UI/출전/재개 →
네 유파·저장 실패·중복·연타·랜덤 결과 재추첨 방지 검사와 실제 입력 검증.
기존 R02/R03/R04의 순서를 이 의존성에 맞춰 교정하며 신규 R 번호/별도 작업 DB를 만들지 않는다.
필수 반증: 흡수 없이 타 유파 책이 나오거나 부여 효과가 인법/다른 장비로 전파됨,
다중 적 타격으로 흡혈이 폭증함, 로드/중복 클릭으로 재추첨/중복 결제됨,
강화 지출 때문에 다른 선택이 항상 열등해짐. 기술 구현 가능, 밸런스/Human NOT_RUN.

2026-09-20 실제 사용처 재확인: 기존 Main은 네 유파 모두 엘리트→흔적→보스→
휴식 Workbench로 이어진다. `test_four_school_battle_contract.gd`의 추가 검사는
실제 제목/유파 선택 후 네 유파의 휴식 표시·전투/시간 중지·미처리 보상 출전 차단·
중복 보스 처리 및 redraw 무변경을 확인한다. 시간 가속/큰 피해를 사용하는 기계
시나리오이며 자연 플레이·새 selected profile2 연결·회복 저장 통과는 아니다.
현재 SchoolCircuit의 기존 scroll/route commit과 selected profile 거래는 다른 경로다.
준비 진입 연결 시 `healing_applied` 불리언만으로 회복 완료를 선언하지 않는다.
실제 HP/회복 결과의 보존·재진입 1회성 및 기존 저장 호환성을 먼저 연결·검증해야 한다.
이번에는 스키마나 회복 수치를 바꾸지 않았다.
...
화면: 새 게임 확인 → 시작 유파 →3택1 두 번→시작 가방/장비 확인→동일 유파 스테이지1 출전.
2026-09-21 사용자 변경으로 신규 첫 전장은 시작 유파와 같다. 기존 독립 첫 전장 저장은 유지한다. 시작 구성은 무기2+선택 인법2지만 보호/이동 인법을
...
**완료 기준(변경):** 4시작 유파의 자동 첫 전장, 나머지3전장6순서씩 총24경로, 기존 독립 전장 저장 호환성, 유파별15시작쌍=60구성의 기계 검사;
...
시험과 실제 렌더를 연결했다. 투사체/표식, WINDUP/LOCKED, 안전경로는
...
후속 공동 슬롯: `EncounterDangerBudget`는 Main별 객체이며 기존 StageProfile의
1/1/2/2 값을 소비한다. start_permission이 허가하기 전에 전조·첫기회 보너스를
소모하지 않는다. recovery 후에도 자식 투사체/소환체가 남으면 점유하며, pause는
동결한다. 배우 사망 통지는 공격 자식을 먼저 정리하고 슬롯을 돌려준다. 발사된
...
2026-09-21 실제 사용처 교정: 장비 단계+비전을 합산하지 않고 곱하던 경로와 투사체
중간 반올림을 수정했다. 8무기×5단계×보조 유무80개 수기 기대값 및 쿠나이 비행 중
장착변경 반례를 확인했다. 무기21/21·874단언 통과. 존재하지 않는 근접/투사 태그
공급자를 인술 피해 수치로 대체하지 않으며 현재 해당 항은0이다. 태그 공급자 전체
구현 완료라는 의미는 아니다. 귀인화 임시검의 별도 수식/원래 무기 복원은 유지한다.

실제 Main에는 apply_committed_backpack 호출이 없었다. cp.backpack에서 확정 가방만
복원해 기존 무기 owner에 전달하도록 연결했다. Continue의 뇌명 비전에서 원타10/
옆 대상0 대신12/6을 검증했다. Retry에서 실패 전투의 탄환과 조합 시계 잔류도 재현해
adopt에만 reset_for_checkpoint를 연결했다. 같은 world+resolver의 탄환/자체 이펙트/
임시 boon·시계만 종료하고, pause는 유지하며, 미리보기/버퍼는 소비하지 않는다.
실제 Main2/2·15단언 통과. 해당 집중 시험은 자연 완주나 모든 빌드의 재미 통과가 아니다.
기존 승인 수식과 사용처를 그대로 연결하는 결함 교정이므로 새로운 게임 규칙/저장
형식/엔진/이미지/과금은 없으며 유효한 내부 근거를 재사용한다.
...
2026-09-21 읽기 개선 증분: PlayerVisualController는 기존 승인 이미지에만2.4초
호흡을 적용하고 이동/피격/사망에서는 neutral로 복귀한다. 피해·본체 위치와 독립이다.
HUD는 실제 HP/오의 숫자, 대시 및 유파별 오의 hover 설명을 읽는다. 적 하단 붉은
체력바는 개별 생명주기/피해 사건에 연결되며 생성 직후 정의된 실제 HP를 반영한다.
선택 인법은 기존 도감 effect_config 설명을 공유한다. 신규 인벤토리 atlas는 LOCK
대기이며 QA 명시적 주입만 가능하다. 소비/미소비 범위와 출처는 CURRENT_VISUAL_HANDOFF
및 후보 README가 소유한다. 이 증분은 신규 프레임 모션·전체 인법/장비 아트·사람
가독성 완료가 아니다. XP 바는 상세규칙의 비도입 결정과 충돌하여 사용자 선택 대기.
...
후속 고정seed921 비교(시작 봉마 인법2개도 동일): 기존1000 p50 193.641/
p95 239.029ms, 원형접촉 p50 56.586/p95 112.842ms. 무기/AI/적수는 유지했다.
멈춘 접촉 몸체의 동일 transform 재대입60회를 별도로 RED 재현하고 정확히 같은
좌표만 대입 생략했다. 집중6/6·34단언, 피해/쿨다운 유지. 이후p95 129.746ms여서
이 작은 guard의 프레임 개선은 입증되지 않았다. 독립 정적 검토 확정P0/P1/P2 없음.
진단 subclass로 플레이어/적 시간을 분리한5초1000표본에서 플레이어298회128.399ms,
적298,000회2989.086ms. 이는 계측 오버헤드 포함 CPU 합계이고 GPU/정상 플레이
증거가 아니다. 밀집 성능은 잔여이며 최대수 제한이나 AI tick 생략으로 우회하지 않는다.
...
당시 Base2회 대비 프로젝트5회 검토 드리프트는 명시 채택으로 유지했으나,
2026-09-20 승인 후 현행 작업 계약 §7의 2회 예산으로 대체했다.
...
96경로 통과·실기기·당시 전체5회 구현 검토·출시를 PASS로 선언하지 않는다.
현재 검토 예산과 이번 R01 검토 결과는 현행 작업 계약 및 Active Context를 따른다.
