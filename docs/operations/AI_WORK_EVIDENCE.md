...
같은9월21일 사용자의 대기모션/설명/체력표시/첫 유파 전장/아이템 이미지 요청을
기존 작업에 이어 반영했다. 신규 출전은 시작 유파 Stage1로 자동 확정하되 기존
교차 유파 저장은 그대로 이어진다. 도감의 실제 인법 효과를 선택지에 재사용하고,
HUD에 실제 HP·오의 숫자 및 무적대시/유파별 오의 hover 안내를 연결했다. EXP는
현재 명시적 비도입 결정과 충돌해 별도 질문 중이며 구슬을 경험치로 이름만 바꾸지 않았다.
기존 승인 캐릭터에2.4초 visual-only 호흡/발 접지 보정을 추가했다. 새 프레임 모션
완료는 아니다. 적마다 독립 붉은 하단 체력바를 연결했다.

Built-in image_gen으로 기본장비3종/가방/4유파 인법서 표지의8개 atlas 후보를
초록 크로마키로 제작한 뒤 같은 도구로 배경을 제거했다. 정확한 프롬프트·원본·
RGBA·해시는 docs/visual/candidates/inventory-icons-20260921/README.md에 보존했다.
alpha검사/Aseprite제한메타조회/명시적QA주입은 수행, 사용자LOCK/생산기본바인딩은
미수행이다.24인법은4표지를 공유하며 추가6무기와 가방 구매 그림은 연결 미완료다.

새8시험은 항목별 RED→GREEN으로 추가했다. 중간 전체970/972 실패는 바뀐 첫 전장/
체력바의 옛 기대값2개였으며 실제 새 동작을 확인하도록 교정했다. 이후976/976,
123스크립트·16,680단언·293.757초·종료0, Python21/21·13.799초 통과.
실제Windows 렌더766371에서 draft/hover/전투/휴식/다음 유파/복구 포인터 검증 통과.
초기 렌더에서27px 체력바 및 비치는 tooltip을 발견해 실제 레이아웃 회귀시험으로
교정했다. idle 두 시점 캡처도 확인했지만 전용대기 프레임·자연 플레이 검증은 아니다.

독립 변경 영향검토의 HP구독 순서 지적을 재현·교정했고, 추가로 기존 일반 적이
정의 적용 전20HP로 생성되는 오류를 발견했다. RED20≠34 뒤 최초 정의의 full-HP만
초기화하도록 교정했다. 피해 받은 적/재설정/사망은 회복시키지 않는다. 집중20/20·
189단언 통과, 독립 제한 검토 새P0/P1/P2 없음. 수치 카탈로그/저장/설치/전역 설정은
불변이다. 마지막 전체·24경로·빌드 결과는 완료 후 이 날짜 기록에 계속 누적한다.

HP교정 후 전체977개 중976통과/1실패: test_mvp3_stage_loop의 처치 반환량 기대가
옛20으로 고정돼 있었으나 봉인 추적자 카탈로그는34였다. 수치를 바꾸지 않고 해당
기대값을34로 교정했다. 같은 실행의 escape fixture 미정리 child2 경고는 별도 집중
7/7·33단언에서는 재현되지 않았다. 경고의 근본 원인을 해결했다고 주장하지 않는다.
생산 기본(후보 atlas 미주입) Windows 포인터/렌더854605 통과 및 오의 tooltip/체력바
화면 확인. export/EXE headless120프레임은 종료0. 추가 EXE --script 포인터 probe는
로그·캡처 없이 완료되지 않아 INCONCLUSIVE로 남기고 그 시험 프로세스만 종료했다.
별도로 켜져 있던 실행은 보호했다. EXE 실제 포인터 검증으로 확대하지 않는다.

새 시작유파 일치24경로(4시작×나머지6순서)는 실제Main/actor/저장/최종정산까지
모두통과·종료0. 시간/피해 가속과UI신호이며 자연플레이가 아니다. 시작한 시점이
HP/표시 후속교정보다 앞서므로 경로·거래 증거로 한정한다. HP기대값 교정 상태 전체
977/977·123스크립트·16,694단언·371.85초 통과. 이전 escape fixture 경고는 이
전체에서는 없었지만 재실행 성공을 원인 교정으로 기록하지 않는다.

큰 봉마보스 bar y34가 그림하단56.43보다 위인 반례를 재현해 그림 경계+4로 교정했다.
Windows render778948에서 큰보스 fixture를 직접 확인했다. 독립 제한 검토에서
최종 보스가 작은형태→봉마로 바뀔 때 높이가 그대로인P2를 찾아 RED36.125<56.43
재현 후 기존 theme_changed 신호에 재배치를 연결했다. 크기 복귀/HP보존 포함
집중16/16·118단언 통과, 지적 교정 readback 종료. 매프레임 군중 순회는 추가하지 않았다.

최종 Windows 후보는 build/windows-playtest-20260921-readable 에 별도출력했다.
기존 settings 폴더 실행이 켜져 있어 삭제/이동하지 않았다. export/EXE headless120
프레임 모두 종료0, PCK SHA25606193F1420BD77EC54AA5464019514C385C59143A1093B3E8C1F3F04C89DFAA9.
Core실제체력이 기존오류20에서 정의26~44로 교정되어 자연난이도 증가 가능성이 있다.
사용자의 직접 플레이/자연속도 밸런스 검증과 최종 모션/새그림LOCK은 미완료다.

이번 최종 theme-aware 상태 전체979/979·123스크립트·16,703단언·310.518초·종료0
통과(readability-theme-final-full-20260921.log). escape fixture의 child2 경고1건은
남아 있어 warning-free 또는 누수교정 완료로 보고하지 않는다. Python21/21·17.577초
통과. PR147은전체제품 gate미완료로 Draft 유지하며 pushed exact-head 원격 검사는
PR실행 이력에서 확인한다. 기존PDF의9월21일 요약으로 누적하며 별도날짜문서를 만들지 않는다.

같은 날 후속 요청: 전체화면 Space 대시가 설정을 여는 문제, 빈 바닥, 대기/새이미지
미연결, 좌상단 인술 쿨타임, 처치 경험치 레벨업, 고정된 검기·수리검과 유파별
몬스터 연결을 교정했다. 경험치 추가는 사용자의 명시 요청이며 이전 비도입 기록을
정정했다. 일반/엘리트/보스1/8/20XP, 비용12+6씩, 인술최대5단계는 초기 시험값이다.
새 책은 해금·활성 인법 제한·실제 가방공간을 통과해야 하며, 원래 출전 재도전
체크포인트를 덮어쓰지 않고 모닥불→다음 출전으로 저장한다.

검기 이동·감쇠와 수리검 방향은 별도 무기 표현으로 연결했다. 기존4유파 시트의
18역할과 기존 봉마 전용2역할을 사용하고 신규 대기 자세1장은 실제 이미지 도구로
크로마키 제작→배경 제거→Aseprite 검사→씬 연결했다. 최초 출력이 크로마 요청을
따르지 않아 녹색 중간본을 별도 생성했다. alpha모서리0, 녹색잔여0, 내부의 대부분
alpha253으로 완전255불투명은 아니며 추출 중 세부 재작성도 있다. 최종 자산LOCK은
주장하지 않는다. 원본·prompt·SHA·consumer는 player-idle-20260921/README.md에 있다.
인법4공유표지/기본장비3/가방 그림은 실제 UI에 연결했다. 추가무기6의 고유 그림과
모든모션 완성은 이 변경의 완료 범위가 아니다.

재현→교정: Space/방향/검기이동/반복면적/대기자세/패드Start를 회귀검사로 확인했다.
처치XP·새 인술 획득·저장 재개·다음 출전·잘못된 획득기록 거부4/4·29단언 통과.
초기 전체992중6실패는 반복1회 고정 가정과 새 레벨 선택을 처리하지 않는 경로였다.
물리 입력 해제 대기를 실제로 통과하도록 교정 후 전체994/994·126스크립트·16,829
단언·369.432초·종료0. 기존 escape fixture unfreed children2 경고1건은 남아 있다.
Python21/21·17.887초 통과. 독립 범위검토 P2두건(패드설정 접근/귀인화 지원기술
억제 오표시)을 수정하고 해당 지적만 재확인했다. 전체검토 횟수 초기화는 없다.

소스1920×1080 전체화면 실제입력/렌더826356 통과. 내보낸 PCK를 같은버전 엔진으로
실행한843892에서도 Space 대시,6000/-4200 이동 후 바닥,대기,20역할 비교,레벨업
마우스 선택 통과. EXE자체는headless120프레임 종료0이며 -s 사용자QA는 실행하지
않아 중지했다. PCK검증을 EXE직접 포인터검증으로 바꾸어 보고하지 않는다.
실행본 build/windows-playtest-20260921-growth, PCK SHA256
510C2420AFA6EAC6CA6E3B6DD7524B27F824B1096F5B80E88F796763A4D265AB.
24경로 실제Main 검사는 아래 후속 결과에서 완료 확인했다.
Human재미/자연속도밸런스/최종그림/1000몹성능/출시 PASS는 아니다. PR147 Draft유지.

원격9618db8/35601796953: Windows빌드 성공, GUT993/994 실패. 검사용1.25 반올림이 Linux1.2/Windows1.3으로 달라 검사용 값만1.26으로 교정했다. 귀인화 지원기술 표시 단언도 추가하고 집중8/8·79단언·0.801초 통과. bae3201/35602413233은 테스트 전 Godot 다운로드 연결이 끊겨 실패; Windows빌드 성공. 실패 이력을 숨기지 않는다.

24가지 실제Main 경로 모두 성장선택→엘리트/흔적/보스→휴식→다음 전장→최종정산 통과, 종료0(growth-route24-20260921.log). 시간/피해를 가속한 기계 검사이지 사람의24회 플레이가 아니다. 축소 표시 기준은 Godot 공식 import/mipmap·CanvasItem 문서(구현 명세 참조)를 확인했다.

ff9cd05/35602733035는994/994 통과 후 해제된 휴식 UI의 지연 포커스 오류로 실패. 재현2/3 실패 확인 후 닫힌 화면 접근을 차단하여 집중3/3·65단언·8.812초·종료0, SCRIPT ERROR 없음. 최종 원격 결과는 PR147 exact-head 검사에 연결한다.

최종 코드1931e93: 로컬995/995·16,847단언·274.373초·종료0; Python21/21·14.385초. 원격35603475554도995/995·16,845단언·186.426초와 Windows빌드 통과. 재발행 PCK의 전체화면 입력/렌더724717 PASS, EXE120프레임 종료0. 최종 PCK SHA256 FBE5BADE6D332E03BD18553C4E0D68CB7B4B320C608444CA40ABAF5D74E02A6D. 앞의510C 해시는 교정 전 이력이며 최신 실행본으로 사용하지 않는다.
