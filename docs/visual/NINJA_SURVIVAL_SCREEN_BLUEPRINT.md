...
현재 bool 반환만으로 상세 이유 전체를 구별할 수 없으므로 UI에서 이유를 추측하지
않는다. 필요하면 기존 runtime에 읽기 전용 사용 가능성 조회를 추가하는 구현안을
검토한다. UI 자체가 충전을 계산하거나 자원을 차감하면 안 된다.

키보드·패드 action 한 개와 화면 버튼은 같은 요청 경로를 사용한다.
키 배치는 충돌 검사 후 구현 패키지에서 정하고 이 문서에서 연결 완료로 주장하지 않는다.
...
| `scripts/ui/hud.gd`, `scenes/ui/hud.tscn`, `project.godot` | 상단 대시·시간·설정과 이동/대시 입력; 오의 연결 없음 | 상단 한 버튼·상태·입력 action, 키/패드/포인터/터치 경로 검증 |
...
엔진 API 수준으로 구현 가능하나 최신 로컬 Godot 실행 파일/세션 확인과 실행 검증은
구현 진입 전에 별도 수행한다. 소스 검사만으로 runtime 가능성을 확정하지 않는다.
