...
오행폭주 ACTIVE: ‘속성 방출 중 · 대시로 중단’, 플레이어 전방의 길게 퍼지는 브레스.
귀인화 ACTIVE: ‘검 전용 · 남은 시간’, 투사/인법은 일시 중단임을 오의 설명에 명시한다.
일반 스킬 버튼은 추가하지 않는다. 천술의 둥근 폭발, 귀인의 원형 맥동은 새 오의 시안에서
사용하지 않는다. 귀인검 크기/잔상과 브레스 폭은 실제 판정에 대응하고 적 전조를 가리지 않는다.

수치와 발동 책임은 상세 규칙 R-ULTIMATE다. 상단 기존 오의 영역 하나에 게이지와
짧은 상태 문구를 함께 표시한다. 일반 인법 단축바·별도 네 유파 게이지는 만들지 않는다.

| 상태 | 표시 / 입력 결과 |
|---|---|
| CHARGING | 현재 자원/비용 · 충전 중; 요청 실패 비용0 |
| READY_NO_TARGET | 준비 완료 · 대상 없음; 유효 적이 범위 안에 오면 READY |
| READY | 오의 사용 가능 · 현재 입력 장치의 키/버튼 표시 |
| ACTIVE | 발동 중 · 남은 지속; 연타/누름 유지 재발동 없음 |
| BLOCKED | 준비/메뉴/사망 중 비활성; 닫기 입력 전투로 전달 금지 |

색만으로 구분하지 않고 텍스트/아이콘을 병행한다. 포커스와 터치 영역은 동일 요청에
연결한다. 완충과 유효 목표 판정은 분리하며 UI가 발동 권한을 소유하지 않는다.
새 래스터 시안·모션·실행 화면은 아직 제작/검증하지 않았다.

## 최신 장비 분리 — 2026-09-11

상세 규칙 R-EQUIPMENT/R-TRACE를 따른다. 준비 화면은 왼쪽 캐릭터/장비 3슬롯,
가운데 실제 가방, 오른쪽 선택 대상 설명/전후 비교로 구분한다. 시작 인법 선택은
유파 선택 뒤 3택1 두 번, 이후 장비3개와 책2개 배치를 확인한다. 슬롯과 가방의
영역 제목을 각각 ‘캐릭터 장비’, ‘인법·보조 가방’으로 표시한다.

```text
캐릭터 장비             인법·보조 가방          선택 대상 설명
[근접: 일본도]           [A][B][빈]              태그 / 현재 효과
[투사: 수리검]           [A][B][빈]              변경 후 / 잃는 효과
[의복: 닌자복]           [빈][빈][빈]            [변경 취소] [확인]

보스 클리어 → 흔적 선택 ┬ 흡수 → 해당 유파 인법 획득 풀 해금
                       └ 장비 강화 → 장착 장비1개 → 해금 포기 확인 → 원자 확정
```

가방 아이템과 장비를 드래그로 혼용하지 않는다. 장비 교체/강화는 비교 후 명시적
확인, 뒤로 가면 소비 없이 원복한다. 키보드/패드 그룹 이동과 터치 탭 경로도 제공한다.
시작 유파는 승인된 예외로 ‘흡수’를 숨기고 장비 대상 선택으로 진입한다.
‘시작 유파 인법은 유지됩니다’ 안내를 표시한다. 위 양자택일은 미해금 타 유파에만
적용하며 강화 확인 때 ‘이 유파의 인법은 이번 런에서 획득할 수 없습니다’를 표시한다.
이미 처리된 흔적은 결과 조회만 가능하다. 흔적 확정과 출전 확정은 별개임을 안내한다.
닌자복 [외형 후보](candidates/ninja-outfit-20260911/README.md)는 알파 실패로 게임 적용 불가.
이 문서는 수정했으나 기존 PDF/생성 화면 그림은 아직 이전 버전이다.
