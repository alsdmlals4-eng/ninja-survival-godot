...
| `COV-ENV-01` | Combat environment | yes | seamless floor + sparse independent props + contact shadows | `Main/BattlefieldBackdrop/FloorTile`, `Main/BattlefieldProps`, unit `GroundShadow` nodes | `IMPLEMENTED_ON_CURRENT_ISOLATED_BRANCH`; source/import + scoped runtime evidence only; Human/device/merge remain unverified |
...
| `RAF-BATTLEFIELD` | battle | `Main/BattlefieldBackdrop/FloorTile`, `Main/BattlefieldProps`, player/enemy/Boss `GroundShadow` | seamless floor, sparse independent props, grounded units | `USER_LOCKED`, `RASTER_IMAGE` | floor tile + prop atlas + contact shadow; IMG-03 retained as historical provenance/rollback | scene/import + scoped runtime render/input; Human/device/merge deferred | `IMPLEMENTED_ON_CURRENT_ISOLATED_BRANCH` |
