...
func _run() -> void:
	...
	rng.seed = 921
	var count := 0
	print("HORDE_PROFILE engine=", Engine.get_version_info().string, " display=", DisplayServer.get_name(), " renderer=", RenderingServer.get_video_adapter_name(), " viewport=", root.size, " vsync=", DisplayServer.window_get_vsync_mode(), " diagnostic_omit_enemy_collision=", omit_collision)
	for requested in [100, 300, 600, 1000]:
		...
		print("HORDE_SAMPLE ", JSON.stringify({"enemies": requested, "observed": get_nodes_in_group("enemies").size(), "frames": frames.size(), "setup_ms": setup_ms,
			"frame_ms_p50": _quantile(frames, 0.5), "frame_ms_p95": _quantile(frames, 0.95), "frame_ms_p99": _quantile(frames, 0.99),
