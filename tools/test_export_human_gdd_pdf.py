...
class HumanGddPdfExporterTests(unittest.TestCase):
    def test_current_visual_and_backpack_authority_do_not_restore_superseded_state(self) -> None:
        ...
        canon_link = re.search(r"\[DEC-037\]\(([^)]+)\)", agents)
        self.assertIsNotNone(canon_link, "AGENTS must route to the protected spatial owner")
        canon_path = (REPOSITORY_ROOT / canon_link.group(1)).resolve()
        self.assertTrue(canon_path.is_relative_to(REPOSITORY_ROOT))
        self.assertTrue(canon_path.is_file())
        self.assertIn("정확히 3×3", canon_path.read_text(encoding="utf-8"))
        ...
        self.assertIn("| `Q-01` | final Human Blueprint PDF review", master_gdd)
        self.assertNotIn("PDF_FINAL_REVIEW_PENDING", master_gdd)
